...
import realm,{ insertNewType} from '../databases/allSchemas';


export class ModalType extends Component{
    constructor(props){
        super(props)
        this.state={
            show:false,
            ...
            <View>
                    <View style={styles.contenu}>
                        ...
                        <TextInput 
                             ...
                             value={this.state.nom_type} autoCorrect={false} placeholder='Type'/>
                        </View>
                    </View>
                    
                    <View style={styles.footer}>
                        <TouchableOpacity style={styles.buttonCont} onPress={() => {
                            if(this.state.nom_type.trim()==""){
                                alert("Fill the textinput");
                                return;
                            }
                            if(this.state.type_isAddnew==true){
                                const newType = {
                                    ref_type:Math.floor(Date.now()/1000),
                                    nom_type:this.state.nom_type
                                }
                                insertNewType(newType).then().catch((error)=>{
                                    alert (`Insert new type error ${error}`);
                                });
                                this.close();
                            }
                            //queryAllCasier();
